...
# So we can change if full size or small in the inspector
export (int, "full size", "small") var kit_size = 0 setget kit_size_change

# 0 = full size pickup, 1 = small pickup

const HEALTH_AMOUNTS = [70, 30] # using KIT_SIZE OF 0 or 1
...
# Used in the setter getter for the export variable
...
# Just to change the value if switch from full size or small
