...
# Initializing states, animation uses states (transferring from one state to another)
# Animation name : [Animation States]
var states = {
	# Important unidle state
	...
	if has_animation(animation_name):
		if current_state != null:
			...
			if animation_name in possible_animations:
				...
				print("AnimationPlayer_Manager.gd - Wanring: Cannot change to ", animation_name, " from ", current_state)
				return false
		else:
			current_state = animation_name
			play(animation_name, -1, animation_speeds[animation_name])
